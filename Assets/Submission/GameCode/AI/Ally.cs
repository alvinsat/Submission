...
public class Ally : MonoBehaviour, IDamageable
{
	...
	public void MeleeAttack()
	{
		...
		for (int i = 0; i < collidersEnemies.Length; i++)
		{
			...
			{
				collidersEnemies[i].gameObject.GetComponent<IDamageable>().ApplyDamage(2f, transform.position);
			}
		}
		StartCoroutine(WaitToAttack(0.5f));
	}

	GameObject tempThrowableProj;
	public void RangeAttack()
	{
		if (doOnceDecision)
		{
			tempThrowableProj = Instantiate(throwableObject, transform.position + new Vector3(transform.localScale.x * 0.5f, -0.2f), Quaternion.identity) as GameObject;
			tempThrowableProj.GetComponent<ThrowableProjectile>().owner = gameObject;
			Vector2 direction = new Vector2(transform.localScale.x, 0f);
			tempThrowableProj.GetComponent<ThrowableProjectile>().direction = direction;
